...
const sortedByRating = () => {
    return products.slice(0).sort(function (a, b) {
        let aRatingReviews = parseInt(a.ratingReviews.replace(" ", ""));
        let bRatingReviews = parseInt(b.ratingReviews.replace(" ", ""));
        return (aRatingReviews < bRatingReviews) ? 1 : (aRatingReviews > bRatingReviews) ? -1 : 0;
    });
};

const sortedByPrice = () => {
    return products.slice(0).sort(function (a, b) {
        let aPriceStr = typeof a.price.newUan === "undefined" ? a.price : a.price.newUan;
        let aPrice = parseInt(aPriceStr.replace(" ", ""));

        let bPriceStr = typeof b.price.newUan === "undefined" ? b.price : b.price.newUan;
        let bPrice = parseInt(bPriceStr.replace(" ", ""));
